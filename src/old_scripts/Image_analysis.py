...
def process_overlap(labels, domains_df, overlap_properties):
    if labels.shape[0] == 2 and np.max(labels[0]) == 1 and np.max(labels[1]) == 1:
        overlap_labels = np.all(labels, axis=0).astype("uint8")
        overlap_props_dict = regionprops_table(
            label_image=overlap_labels, properties=overlap_properties
        )
        ...
        overlap_props_df["volume"] = overlap_props_df["area"].apply(
            lambda a: a * VOXEL_VOLUME
        )
        ...
        overlap_props_df["overlap_fraction"] = abs(overlap_props_df.at[0, "volume"]) / (
            abs(
                domains_df.loc[
                    (domains_df["Channel ID"] == 0)
                    & (domains_df["roi_type"] == "domain"),
                    "volume",
                ].values[0]
            )
            + abs(
                domains_df.loc[
                    (domains_df["Channel ID"] == 1)
                    & (domains_df["roi_type"] == "domain"),
                    "volume",
                ].values[0]
            )
        ...
        overlap_props_df["distance3d"] = sqrt(
            overlap_props_df.distance_x**2
...
def process_image(
    ...
    overlap_props_df, overlap_labels = process_overlap(
        labels=domain_labels, domains_df=rois_df, overlap_properties=overlap_properties
    )
...
def run():
    files_list = [
        ...
        if f.endswith(IMAGE_FILE_EXTENSION)
        and not f.endswith(f"ROIs.{IMAGE_FILE_EXTENSION}")
    ]
    ...
    for img_file in files_list:
        ...
        rois_df, domain_labels, subdomain_labels, overlap_labels = process_image(
            image=image,
    ...
    metadata_df = pd.read_csv(
        os.path.join(INPUT_DIR, f"{assay_id}_assays.csv"), header=1
    )  # TODO:
