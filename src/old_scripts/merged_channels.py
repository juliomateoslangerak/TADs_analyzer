...
def get_ome_xml(
    ...
    **ome_xml_kwargs,
    ...
    ome_element = ElementTree.Element(
        "OME",
        attrib={
            "xmlns": ome_namespace,
    ...
    instrument_element = ElementTree.SubElement(
        ome_element, "Instrument", attrib={"ID": "Instrument:0"}
    )
    objective_element = ElementTree.SubElement(
        instrument_element,
        "Objective",
        attrib={
            "Correction": "Other",
    ...
    pixels_element = ElementTree.SubElement(
        image_element,
        "Pixels",
        attrib={
            "ID": "Pixels:0",
    ...
    for channel_id in range(size_c):
        channel_element = ElementTree.SubElement(
            pixels_element,
            "Channel",
            attrib={
                "ID": "Channel:0:{:d}".format(channel_id),
                "SamplesPerPixel": str(size_s),
                "EmissionWavelength": str(
                    ome_xml_kwargs["Channel"]["EmissionWavelength"][channel_id]
                ),
                "EmissionWavelengthUnit": ome_xml_kwargs["Channel"][
                    "EmissionWavelengthUnit"
                ][channel_id],
                "ExcitationWavelength": str(
                    ome_xml_kwargs["Channel"]["ExcitationWavelength"][channel_id]
                ),
                "ExcitationWavelengthUnit": ome_xml_kwargs["Channel"][
...
try:
    ...
    for INPUT_DIR in INPUT_DIRS:
        ...
        with open(
            os.path.join(INPUT_DIR, "assay_config.json"), mode="r"
        ) as config_file:
        ...
        for file_root in files_set:
            ...
            for ch in range(1, nr_channels):
                new_channel = tifffile.imread(
                    os.path.join(INPUT_DIR, f"{file_root}_C{ch + 1}.tif")
                )
                image = np.stack((image, new_channel))
            if nr_channels == 1:
                image = np.expand_dims(
                    image, 0
                )  # adding non existing channel dimension
            ...
            if nr_channels < raw_image.getSizeC():
                excitation_wavelengths = [
                    raw_channels[ch].getExcitationWave()
                    for ch in config["raw-crop_channel_mapping"]
                ]
                emission_wavelengths = [
                    raw_channels[ch].getEmissionWave()
                    for ch in config["raw-crop_channel_mapping"]
                ]
            elif nr_channels == raw_image.getSizeC():
                excitation_wavelengths = [
                    raw_channels[ch].getExcitationWave() for ch in range(nr_channels)
                ]
                emission_wavelengths = [
                    raw_channels[ch].getEmissionWave() for ch in range(nr_channels)
                ]
            else:
                raise Exception(
                    "There were more channels in the crop than in the raw image"
                )

            channel_names = [
                CHANNEL_NAME_MAPPINGS[str(raw_image.getChannelLabels()[c])]
