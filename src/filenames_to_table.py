...
def run(assays_directory, image_files_extension, token_sep="_"):
    ...
    for assay_dir in assay_dirs:
        try:
            print(assay_dir)
            with open(
                os.path.join(assay_dir, "assay_config.json"), mode="r"
            ) as config_file:
            ...
            files_list = [
                f for f in os.listdir(assay_dir) if f.endswith(image_files_extension)
            ]
            ...
            with open(
                os.path.join(assay_dir, f"{assay_id}_assays.csv"), mode="w"
            ) as csv_file:
...
if __name__ == "__main__":
    run(assays_directory=INPUT_IMAGE_DIR, image_files_extension=IMAGE_FILE_EXTENSION)
    print("done")
